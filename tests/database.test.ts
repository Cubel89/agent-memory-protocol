...
// ════════════════════════════════════════════════════════
// v2.0.0 Tests: sqlite-vec, sanitizeFtsQuery, hybrid search
// ════════════════════════════════════════════════════════

import * as sqliteVec from "sqlite-vec";

// Helper: sanitizeFtsQuery (copied from database.ts for standalone testing)
...
describe("sanitizeFtsQuery", () => {
  it("replaces hyphens with spaces", () => {
    expect(sanitizeFtsQuery("agent-memory-protocol")).toBe("agent OR memory OR protocol");
  });

  it("removes special FTS5 characters", () => {
    expect(sanitizeFtsQuery('test(foo) "bar" *wild')).toBe("test OR foo OR bar OR wild");
  });

  it("returns null for empty query", () => {
    expect(sanitizeFtsQuery("")).toBeNull();
    expect(sanitizeFtsQuery("-")).toBeNull();
    expect(sanitizeFtsQuery("a b")).toBeNull(); // single-char words filtered
  });

  it("handles normal queries", () => {
    expect(sanitizeFtsQuery("flutter error crash")).toBe("flutter OR error OR crash");
  });
...
describe("sqlite-vec integration", () => {
  ...
  it("creates vec0 virtual table", () => {
    ...
    `);
    // Should not throw
    const count = (db.prepare("SELECT COUNT(*) as c FROM vec_test").get() as any).c;
    expect(count).toBe(0);
  });

  it("inserts and searches vectors (KNN)", () => {
    db.exec(`
      CREATE VIRTUAL TABLE vec_test USING vec0(
        item_id INTEGER PRIMARY KEY,
        embedding float[4] distance_metric=cosine
    ...
    const insert = db.prepare("INSERT INTO vec_test(item_id, embedding) VALUES (?, ?)");
    // Insert 3 vectors
    insert.run(BigInt(1), new Float32Array([1, 0, 0, 0]));
    insert.run(BigInt(2), new Float32Array([0, 1, 0, 0]));
    insert.run(BigInt(3), new Float32Array([0.9, 0.1, 0, 0])); // close to vector 1

    // Search for closest to [1, 0, 0, 0]
    const results = db.prepare(`
      SELECT item_id, distance FROM vec_test
      WHERE embedding MATCH ? AND k = 3
    `).all(new Float32Array([1, 0, 0, 0])) as any[];
    ...
    // Second should be item 3 (close)
    expect(results[1].item_id).toBe(3);
    // Third should be item 2 (far)
    expect(results[2].item_id).toBe(2);
  });
  ...
  it("deletes vectors", () => {
    ...
    db.prepare("DELETE FROM vec_test WHERE item_id = ?").run(BigInt(1));
